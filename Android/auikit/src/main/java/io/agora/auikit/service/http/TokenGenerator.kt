...
object TokenGenerator {
    ...
    fun generateMultiChannelTokens(
        ...
        scope.launch(Dispatchers.Main) {
            try {
                ...
                tokens.forEach {
                    if (it.id == AUiRoomConfig.TOKEN_RTM_VOICE_LOGIN){
                        val fetchToken = fetchToken(it.channelName, it.uid, it.genType)
                        val token = JSONObject(fetchToken)
                        out[it.id] = token["rtmToken"] as String?
                        out[AUiRoomConfig.TOKEN_RTC_VOICE_SERVICE] = token["rtcToken"] as String?
                    }else{
    ...
    fun generateVoiceToken(
        ...
        scope.launch(Dispatchers.Main) {
            try {
                success.invoke(fetchToken(channelName, uid, genType))
            } catch (e: Exception) {
                failure?.invoke(e)
            }
        }
    ...
    private suspend fun fetchToken(
        channelName: String, uid: String, genType: TokenGeneratorType) = withContext(Dispatchers.IO) {
        val postBody = JSONObject()
        postBody.put("channelName", channelName)
        postBody.put("userId", uid)

        val request = Request.Builder().url(
            if (genType == TokenGeneratorType.token006) "https://uikit-voiceroom-staging.bj2.agoralab.co/v1/token006/generate"
            else "https://uikit-voiceroom-staging.bj2.agoralab.co/v1/token/generate"
        ...
        if (execute.isSuccessful) {
            ...
            if (bodyJobj["code"] != 0) {
                ...
                (bodyJobj["data"] as JSONObject).toString()
            }
        } else {
    ...
    private suspend fun fetchToken(
        ...
        ).addHeader("Content-Type", "application/test.json").post(postBody.toString().toRequestBody()).build()
        val execute = okHttpClient.newCall(request).execute()
        if (execute.isSuccessful) {
            val body = execute.body
                ?: throw RuntimeException("Fetch token error: httpCode=${execute.code}, httpMsg=${execute.message}, body is null")
