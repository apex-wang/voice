...
class AUIGiftListAdapter constructor(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (convertView == null) {
            convertView = View.inflate(context, R.layout.voice_item_gift_list_layout, null)
        }
        ...
        convertView?.let {
            ...
            ThreadManager.getInstance().runOnMainThread{
                Glide.with(context).load(giftInfo?.giftIcon).error(R.drawable.voice_icon_gift_1).into(img)
            }
        ...
        if (isSelected){
            ...
            layout.background = context.let {
                ContextCompat.getDrawable(
                    it,
                    R.drawable.voice_bg_gift_selected_shape
                )
            }
        }else{
