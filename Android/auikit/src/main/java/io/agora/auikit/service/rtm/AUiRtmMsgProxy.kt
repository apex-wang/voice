...
class AUiRtmMsgProxy : RtmEventListener {
    ...
    fun subscribeMsg(channelName: String, itemKey: String, delegate: AUiRtmMsgProxyDelegate) {
        Log.e("apex","subscribeMsg: $channelName - $itemKey")
    ...
    override fun onStorageEvent(event: StorageEvent?) {
        ...
        event.data.metadataItems.forEach { item ->
            ...
            Log.d("rtm_event", "onStorageEvent: key event:  $delegateKey  ${item.key} \n value: ${item.value}")
            msgDelegates[delegateKey]?.forEach { delegate ->
    ...
    override fun onMessageEvent(event: MessageEvent?) {
        Log.d("apex", "onMessageEvent update: ${event?.topicName} - ${event?.publisher}")
        event ?: return
        val str = event.message?.let { String(it) }
        Log.d("apex", "onMessageEvent update: $str")
