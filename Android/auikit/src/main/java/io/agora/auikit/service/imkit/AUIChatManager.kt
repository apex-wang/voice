package io.agora.auikit.service.imkit
...
class AUIChatManager(
    ...
    private val logger = AUiLogger(AUiLogger.Config(context, "AUIChatManager"))
    ...
    private var giftMap:MutableMap<String,ArrayList<AUIGiftEntity>> = mutableMapOf()


    init {
        chatClient.chatManager().addMessageListener(this)
        chatClient.chatroomManager().addChatRoomChangeListener(this)
    }
    ...
    fun initManager(channelId:String?,roomContext: AUiRoomContext){
    ...
    fun updateGiftMsgCache(channelId:String?,gift:AUIGiftEntity){
        currentGiftList.clear()
        val giftList = ArrayList<AUIGiftEntity>()
        val arrayList = giftMap[channelId]
        gift.let {
            currentGiftList.add(it)
            if (arrayList != null && arrayList.size > 0){
                giftList.addAll(arrayList)
            }
            giftList.add(it)
            if (channelId != null){
                giftMap[channelId] = giftList
            }
        }
    }

    fun getChatUser(user:String){
