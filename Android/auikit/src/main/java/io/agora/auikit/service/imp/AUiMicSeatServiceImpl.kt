...
import io.agora.auikit.service.http.seat.*
...
class AUiMicSeatServiceImpl(
    ...
    override fun onClickInvited(index: Int) {
        Log.e("apex","AUiMicSeatServiceImpl onClickInvited")
        ...
        seats?.values?.forEach {
            ...
            delegateHelper.notifyDelegate { delegate ->
                ...
                if ((originMicSeat?.user == null || originMicSeat.user?.userId?.length == 0)
                    && newUser != null) {
                    Log.d("mic_seat_update", "onAnchorEnterSeat: $it  onMsgDidChanged: $index")
